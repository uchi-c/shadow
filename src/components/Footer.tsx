...
export default function Footer({ onNavigate, onOpenAdmin }: FooterProps) {
  ...
  return (
    <footer className="bg-[#0b0f14]/95 text-white pt-16 pb-12 overflow-hidden border-t border-[#2563eb33] relative">
      <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-[#2563eb] to-transparent"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-10 grid grid-cols-1 md:grid-cols-12 gap-10">
        ...
        <div className="md:col-span-3 space-y-4 font-sans">
          ...
          <ul className="text-slate-400 text-xs space-y-2.5">
            ...
            <li>
              <button onClick={() => onNavigate("services")} className="hover:text-[#2563eb] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#2563eb] hover:underline transition-all cursor-pointer rounded px-1 text-left">
                Capabilities (Services)
              </button>
            </li>
            <li>
              <button onClick={() => onNavigate("about")} className="hover:text-[#2563eb] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#2563eb] hover:underline transition-all cursor-pointer rounded px-1 text-left">
                The Founder Story
              </button>
            </li>
            <li>
              <button onClick={() => onNavigate("case")} className="hover:text-[#2563eb] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#2563eb] hover:underline transition-all cursor-pointer rounded px-1 text-left">
                Completed Missions (Portfolio)
              </button>
            </li>
            <li>
              <button onClick={onOpenAdmin} className="hover:text-[#2563eb] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#2563eb] hover:underline transition-all cursor-pointer flex items-center space-x-1.5 font-bold text-[#60a5fa] rounded px-1 text-left">
                <KeyRound className="w-3.5 h-3.5 shrink-0 text-[#2563eb]" />
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-10 border-t border-white/5 mt-12 pt-6 flex flex-col md:flex-row sm:items-center justify-between text-[10px] text-slate-500 font-sans">
        ...
        <div className="flex items-center space-x-4 mt-4 md:mt-0">
          <button onClick={scrollToTop} className="hover:text-[#2563eb] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#2563eb] rounded px-1 flex items-center space-x-1 font-mono hover:underline" aria-label="Scroll immediately to key page header navigation">
            <span>Scroll to Surface</span>
            <ArrowUp className="w-3.5 h-3.5 shrink-0 text-[#2563eb]" strokeWidth={2.5} />
          </button>
        </div>
      </div>
    </footer>
