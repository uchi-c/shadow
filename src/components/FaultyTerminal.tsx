...
export default function FaultyTerminal({
  ...
  useEffect(() => {
    ...
    // Honour the user's motion preference: render one frame, never loop.
    const reduceMotion =
      typeof window !== 'undefined' && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    const effectiveMouseReact = mouseReact && !reduceMotion;

    let renderer: Renderer;
    try {
      renderer = new Renderer({ dpr });
    } catch {
      return; // WebGL unavailable — the surrounding section still renders fine.
    }
    ...
    const update = (t: number) => {
      ...
      if (pageLoadAnimation && !reduceMotion && loadAnimationStartRef.current > 0) {
